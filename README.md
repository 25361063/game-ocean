...
第一人称深海探索、多关卡闯关、局内构筑与巨兽战斗小游戏。当前版本为 **v18 完整通关校验版**。

## v18 修复

- 使用真实 Chrome 从第 1 关连续推进并完成第 20 关，覆盖 19 次跨关、40 段关末/关初剧情和第一部专属结算
- 修复第 10、18 关防守任务完成后，玩家离开防守圈前往舱门会导致进度倒退、出口重新锁定的软锁
- 防守进度达到要求后现在会永久锁定为完成状态，同时给出分数、音效和舱门解锁提示
- 修复第 20 关剧情结束后结算界面已显示、但内部状态仍停在暂停态的问题；现在会正确进入 `OVER` 状态
- 修正调试状态读取，使第一部完成结局与分支结局都能正确报告
- 完整通关回归结果：20/20 关通过、氧气逐关刷新、防守完成锁定、终章状态及文案正确、控制台零错误
